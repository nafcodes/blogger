...
(function(win, doc) {
    ...
    function ad_set(over) {
        ...
        load(blogger('6681295800241947021') + '?alt=json&max-results=0&callback=_' + fn);
    }
    ...
    win['_' + (fn + 1)] = function($) {
        ...
        var entry = $.entry || [],
            ...
            ss = ' style="display:block;width:100%;height:auto">',
            i, j, s;
        if (!entry_length) return;
        for (i = 0; i < entry_length; ++i) {
            j = entry[i];
            s = 'media$thumbnail' in j ? '<img alt="" src="' + j.media$thumbnail.url.replace(/\/s\d+(-c)?\//, '/w1600-h675-p-k-no-nu/') + '" width="1600" height="675"' + ss : '<span class="img"' + ss + '</span>';
            var url = (j.link.find(function($) {
                return $.rel === "alternate";
            }) || {}).href;
            if (!url) continue;
            s += '<header><h3><a href="' + url + '" target="_blank">' + $.title.$t + ' &middot; ' + j.title.$t + '</a></h3></header>';
            s += '<p>' + j.summary.$t.replace(/<.*?>/g, "").replace(/[<>]/g, "").slice(0, 200) + '&hellip;</p>';
            insert(ul, el('section', s));
        }
        insert(content, ul);
    ...
    win['_' + fn] = function($) {
        ...
        load(blogger('6681295800241947021') + '?alt=json&orderby=updated&start-index=' + i + '&max-results=10&callback=_' + (fn + 1));
    };
